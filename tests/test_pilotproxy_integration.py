...
def runtime():
    binary = os.environ.get("PILOTPROXY_TEST_BINARY")
    bundle = os.environ.get("PILOTPROXY_TEST_BUNDLE")
    if not binary and not bundle:
        pytest.skip(
            "set PILOTPROXY_TEST_BINARY and PILOTPROXY_TEST_BUNDLE for CUDA tests"
        )
    assert binary and bundle, "both PilotProxy test paths must be set"
