...
def run(tmp_path, period, fault=None):
    ...
    for reader in readers.values():
        reader.write()
    refused = fault == "missing-time"
    skipped = 1 if fault and not refused else 0
    out = runner.DumpChordBuffer(
        str(tmp_path),
        (1, F, P, D // 8, 6),
        np.uint32,
        max_frames=FRAMES - skipped,
    ...
    if refused:
        return stage
    actual = out.load()
    assert len(actual) == FRAMES - skipped
    if skipped:
        return stage
...
def test_missing_identity_refused(tmp_path):
    result = run(tmp_path, 4, "missing-time")
    assert result.return_code != 0
    assert "missing time/frequency identity" in result.output
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("decision", "decision is not 0 or 1"),
        ("upchannelized", "frequency identity mismatch"),
    ],
)
def test_bad_block_skipped(tmp_path, fault, diagnostic):
    # One bad block is skipped with a warning; the recorder keeps going and
    # re-synchronises on the next good block instead of taking the run down.
    result = run(tmp_path, 4, fault)
    assert result.return_code == 0, result.output[-4000:]
    assert f"DtvInputHealth {diagnostic}; skipping the block" in result.output
    assert result.output.count("skipping the block") == 1
