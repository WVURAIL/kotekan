...
`COMMIT` and `VENDOR.json` identify the upstream revision; the manifest also
records each file's SHA-256 digest.
...
revision here, and update both pins and the file hashes. Check with:
...
through `fstat_get_config`. Other kernel settings keep their upstream defaults.

The API uses the legacy default CUDA stream. `cudaPilotProxyDetector` uses
events to order those calls with the pipeline streams. Its local `config.h`
is included with quotes by the vendored files, so it does not replace other
headers with that name.

This README, the CMake file, the manifest and the commit pin are maintained in
