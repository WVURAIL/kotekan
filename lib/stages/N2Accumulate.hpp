 ...
 *
 * Counts must be between zero and sub_integration_ntime. The redundant upper entries in
 * diagonal tiles are ignored. In the default scalar mode, counts must also be equal across
 * products within each subintegration and frequency. Unequal counts require
 * packet_loss_is_scalar: false, variance_mode: EvenOddPosDef, and an output descriptor with
 * support_mode: per_product_v1. Debug accumulation is not supported in that mode.
 *
 ...
 * converted from voltage samples to FPGA ticks at output and bounded by the frame interval.
 * The scalar valid, packet-loss, RFI and RFI-only counters are zeroed to mark them
 * unavailable, and scalar loss-fraction metrics are omitted; incoming scalar diagnostic counts
 * are still checked for synchronisation and range.
 *
...
private:
    ...
    std::vector<int32_t> _n_valid_samples_in_vis;
