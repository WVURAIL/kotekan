...
#include "kotekanLogging.hpp"    // for FATAL_ERROR, DEBUG, FATAL_ERROR_NON_OO, INFO, WARN
...
N2Accumulate::N2Accumulate(Config& config, const std::string& unique_name,
    ...
    n_pl_gauge(Metrics::instance().add_gauge("kotekan_N2accumulate_frac_flagged_fpga_ticks_pl",
                                             unique_name, {"freq_id"})),
    n_rfi_gauge(Metrics::instance().add_gauge("kotekan_N2accumulate_frac_flagged_fpga_ticks_rfi",
                                              unique_name, {"freq_id"})),
    n_rfi_only_gauge(Metrics::instance().add_gauge(
        "kotekan_N2accumulate_frac_flagged_fpga_ticks_rfi_only", unique_name, {"freq_id"})) {
    ...
    _n_valid_samples_in_vis = std::vector<int32_t>(_num_freq_per_n2k_frame, 0);
...
void N2Accumulate::main_thread() {
    ...
    auto& comp_time_seconds_metric =
        Metrics::instance().add_gauge("kotekan_N2accumulate_accum_time", unique_name);
    auto& samples_in_out_frame = Metrics::instance().add_gauge(
        "kotekan_N2accumulate_samples_in_accumulated_out_frame", unique_name);
    auto& skipped_input_frames = Metrics::instance().add_gauge(
        "kotekan_N2accumulate_skipped_frames", unique_name, {"freq_id", "reason"});
    ...
    bool warned_support_identity = false;
    ...
    while (!stop_thread) {
        ...
        if (!_packet_loss_is_scalar) {
            ...
            if (!support_identity.is_null() && identity != support_identity
                && !warned_support_identity) {
                WARN("N2Accumulate per-product dataset or RFI policy changed mid-run; later "
                     "frames use the new identity");
                warned_support_identity = true;
            }
        ...
        if (have_previous_seq && frame_seq != previous_seq + _fpga_ticks_per_n2k_frame) {
...
#pragma omp parallel for num_threads(_num_workers)
            ...
            for (int64_t f = 0; f < _num_freq_per_n2k_frame; ++f) {
                ...
                _n_valid_samples_in_vis[f] += count_t0 + count_t1;
            ...
            if (next_bin_idx != _accum_bin_idx) {
                ...
                // set skipped frame metrics and reset counter
                for (int64_t f = 0; f < _num_freq_per_n2k_frame; ++f) {
                    skipped_input_frames.labels({std::to_string(f), "rfi"})
...
void N2Accumulate::accum_per_product(int64_t f, const int32_t* corr0, const int32_t* corr1,
    ...
    for (int64_t i = 0; i < _num_elements; ++i) {
        for (int64_t j = 0; j <= i; ++j) {
            // Correlation tiles are 16x16; the matching packet-loss count tile is 8x8,
            // since masks group eight dishes per polarization in native [P,D] order.
            const int64_t ib = i / 16, jb = j / 16;
            const int64_t p = 256 * (ib * (ib + 1) / 2 + jb) + 16 * (i % 16) + j % 16;
            const int64_t gi = i / 8, gj = j / 8, gb = gi / 8, hb = gj / 8;
            const int64_t c = 64 * (gb * (gb + 1) / 2 + hb) + 8 * (gi % 8) + gj % 8;
...
bool N2Accumulate::output_and_reset(frameID& in_frame_id, frameID& in_rfiframemask_frame_id,
    ...
    if (_vis_samples_in_out_frame < 0 || _fpga_ticks_per_n2k_correlation <= 0) {
        FATAL_ERROR("N2Accumulate invalid output tick span: {} samples, {} ticks per correlation",
                    _vis_samples_in_out_frame, _fpga_ticks_per_n2k_correlation);
    }
