...
/**
 * Combine one CHORD 8192-sample DTV block with an aligned RFImask frame.
 *
 * DTV int8[F] uses 1=reject. RFImask uint1x8[8,F,128] uses 1=keep.
 * The output is their intersection: a DTV rejection clears every time bit
 * of that frequency, and a DTV acceptance preserves the existing mask.
 * Both the visibility correlator and its packet-loss counter must consume
 * this output. Missing decisions block processing; mismatched metadata stops it.
 *
 * One detector block per RFImask frame is supported.
 */
...
public:
    ...
    void main_thread() override {
        ...
        while (!stop_thread) {
            ...
            if (seq < 0 || dp <= 0 || seq > std::numeric_limits<int64_t>::max() - dp || rp <= 0
                || rp % 1024 || int64_t(rp) * 8 != dp || seq != dm->get_fpga_seq_num())
            ...
            for (int f = 0; f < frequencies; ++f)
                if (reject[f] > 1)
                    FATAL_ERROR("DtvRfiMask decision must be 0 or 1");
