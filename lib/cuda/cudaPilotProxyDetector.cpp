 ...
 * CUDA events order the library's default-stream work with the pipeline streams.
 *
 * @par GPU Memory
...
void cudaPilotProxyDetector::bind_first_frame(
    ...
    // One staging region per pilot channel, shared by pipelined instances.
    // The next packer waits for the previous block's default_stream_done event.
...
cudaEvent_t cudaPilotProxyDetector::execute(cudaPipelineState& /*pipestate*/,
    ...
    // Hand the packed input to the vendored library's legacy default stream.
